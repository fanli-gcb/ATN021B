...
# load emmeans results (GROUP 2 vs Group 4a+4b, FDR < 0.1)

emmeans_all <- read.xlsx(
  "emmeans/GROUP2_vs_GROUP4a+4b_emmeans_noARV.xlsx"
)
...
boruta_all <- fread(
  "boruta_imps_GROUP2_vs_4ab_unadjusted_noARVs.csv"
) %>% as.data.frame()
...
# pull emmeans rows + add pathway columns

emmeans_overlap <- emmeans_sig %>%
  filter(metabolite %in% overlap_names) %>%
  left_join(metabolon_map,
            by = c("metabolite" = "CHEMICAL.NAME")) %>%
  relocate(metabolite, SUB.PATHWAY, SUPER.PATHWAY)


# pull Boruta rows + add pathway columns
...
saveWorkbook(wb,
             file      = "emmeans/Supp_Table_Overlap_GROUP2_vs_4ab.xlsx",
             overwrite = TRUE)

message("Done. Saved: emmeans/Supp_Table_Overlap_GROUP2_vs_4ab.xlsx")
